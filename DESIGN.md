...
`StaggeredMenu` 以透明页头悬浮在扫描空间上方，左侧社团标志为 36.8px 正方形、9px 圆角，并链接到 `#top` 以准确返回初始首页位置；右侧“社团导航”按钮与加号整体放大 15%。展开时，纯白、中灰与纯黑三层以 75ms 间隔、540ms `power3.out` 依次滑入，面板以 600ms `power3.out` 跟进；面板就位时，三层保持 490ms 时长和原有错落节奏，但改为在隐藏容器内横向扫出，不再整面同时淡出，避免收尾亮度突变。关闭面板恢复使用 340ms `power3.inOut`，颜色层会先在 160ms 内隐去，并以 110% 位移完全退出视口，避免收尾闪动或移动端边缘残留。按钮文字按需生成切换序列，静止时只保留当前一行，避免裁切边界露出下一行笔画。每次打开前必须把 panel 与所有颜色层的 opacity 重置为 1，并允许打开动作中断关闭 tween，保证连续或快速开关都能完整重播。最终菜单不使用毛玻璃或不透明底色，而是在 420px 宽度内使用六段透明渐变：左侧完全透明，向右依次为 3.5%、9%、16%、24%，最右侧仅 32% 黑色遮罩。内容左侧留白为 80px，避免菜单文字与首页标题直接叠压；文字使用局部黑色柔光，而非整块底色。菜单项按 01–04 编号错落进入，字距为 0.1em；641–1024px 保持全屏面板，640px 以下改为右上角的 280×164px 紧凑面板，菜单文字降为 21.6px，内容与动效不变。点击链接、点击外部或按 Esc 均关闭菜单；站内跳转在顶部预留 88px，并隐去程序性焦点的默认轮廓。

### GridScan / EchoText

`GridScan` 是全屏固定 WebGL 透视网格，随指针产生视角响应并以白色扫描光穿越空间。首页使用适度增强但不干扰阅读的参数：灵敏度 0.43、扫描透明度 0.22、3.7 秒扫描周期、0.9 秒间隔、0.29 bloom、0.0009 色差和 0.005 噪点，并通过 1.08 倍亮度让网格层次更清晰。`EchoText` 通过 12 层滞后副本产生方向性回声。两者在减少动态效果模式下分别退化为静态网格与单层文字，核心内容始终可见。
