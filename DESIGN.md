...
`GridScan` 是全屏固定 WebGL 透视网格，随指针产生视角响应并以白色扫描光穿越空间。首页使用更克制的低干扰参数：灵敏度 0.38、扫描透明度 0.21、3.6 秒扫描周期、0.9 秒间隔、0.28 bloom、0.0009 色差和 0.005 噪点。`EchoText` 通过 12 层滞后副本产生方向性回声。两者在减少动态效果模式下分别退化为静态网格与单层文字，核心内容始终可见。
